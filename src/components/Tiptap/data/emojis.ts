 ...
 */
const emojisWithShortcodes = joinShortcodes(emojis as CompactEmoji[], [
  shortcodes,
]);
...
emojisWithShortcodes.forEach(({ unicode, emoticon, shortcodes }) => {
   ...
   */
  if (emoticon) {
    emoticonsForRegex.push(escapeRegExp(emoticon.toString()));
    EMOJI_EMOTICONS[emoticon.toString()] = unicode;
  }
